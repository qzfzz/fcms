		...
		switch( $itype )
		{
			case 2:
				...
				$this->di->set( 'viewCache', function() use( $info ){
					...
					$cache = new MemcacheBackend( $frontCache, $this->config->memcache->backend->toArray() );
					return $cache;
				});
			break;
			case 1:
			...
			case 4:
				...
				$this->di->set( 'viewCache', function() use( $info ){
					...
					$cache = new \Phalcon\Cache\Backend\Mongo($frontCache, $this->config->cache->mongodb->backend );
					return $cache;
				});
			break;
		}
