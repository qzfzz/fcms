                        ...
                        <div class="col-xs-8">
                            {% for key,group in groups %}
                            	<label class="radio-inline">
                            		<input type="radio" name="groupId" value="{{ group[ 'id' ] }}"/>{{ group[ 'name' ] | e }}
                            	</label>
                            {% endfor %}
                        </div>
                    </div>
                ...
                $( 'input' ).blur( function(){
                    ...
                    if( value )
                    {
                        ...
                        else if( 'email' === id ) //是邮箱，就去验证邮箱是否正确
                        {
                            ...
                            {
                            	objParent.find( 'span' ).remove();
                            	var msg = '<span id="helpBlock" class="help-block text-left" style="">邮箱格式非法</span>';
                                objParent.append( msg );
                                error( objParent );
                                return false;
                            }
                        ...
                        else if( 'password' === id ) //判断密码是否符合6-15位且不包含非法字符
                        {
                        	...
                        	{
                        		objParent.find( 'span' ).remove();
                                var msg =  '<span id="helpBlock" class="help-block text-left" style="">请输入6-15位只包含数字、字母、下划线或特殊字符的密码！</span>';
                                objParent.append( msg ); 
                                error( objParent );
                        ...
                        else if( 'groupId' === id ) //是所属分组
                        {
                            ...
                            {
                            	objParent.find( 'span' ).remove();
								var msg = '<span id="helpBlock" class="help-block text-left" style="">未选择用户组</span>';
								objParent.append( msg ); 
                                error( objParent );
                                return false;
                            }
					...
					{
						objParent.find( 'span' ).remove();
						var msg = '<span id="helpBlock" class="help-block text-left" style="">数据不能为空</span>';
						objParent.append( msg );
                        error( objParent );
					}
                });
