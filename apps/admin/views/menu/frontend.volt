    ...
    <body class="wrap">
                        ...
                        {% for i,item in page.items %}
                        	<tr id="menu{{item.id}}">
	                        	...
	                        	<td style="text-align:center">
	                        		{% if item.pname is not empty %}{{ item.pname | e }}{% endif %}
	                        	</td>
        ...
        $(function(){
        	...
        	$( 'i.menuDelete' ).click(function(){
        		...
        		$.confirm( { title: '确定删除该菜单?', confirm: function(){
					var optid = $( 'i.menuDelete' ).parent().parent( 'td' ).attr( 'data-id' );
	        		if( false == optid ){
	        		...
	        		$.get( '/admin/menu/deleteMenu/id/' + optid, function( ret ){
	        			if( 1 != ret.state ){
	        				 $( '#dis_message' ).html( ret.msg );
		   	       			 $( '#dis_message' ).parent().parent().removeClass( 'alert-danger' ).addClass( 'alert-success' );
		   	   				 $( '#dis_message' ).parent().parent().fadeIn( 500 );
	    	        		 ...
	    	        		 $( '#dis_message' ).html( ret.msg );
	    	       			 $( '#dis_message' ).parent().parent().removeClass( 'alert-success' ).addClass( 'alert-danger' );
	    	   				 $( '#dis_message' ).parent().parent().fadeIn( 500 );
	    	   				 setTimeout( function(){
	    	   				 	 $( '#dis_message' ).parent().parent().fadeOut( 1000 );
	    	   				 }, '2000' );
