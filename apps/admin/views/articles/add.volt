...
<html>
    ...
    <body class="wrap">
        <ul class="nav nav-tabs" role="tablist" id="tabs">
            ...
            <li role="presentation" id="artcontent"><a href="#articleContent">添加文章内容</a></li>
        </ul>
								...
								<tbody>
									<tr>
										...
										{% if tags is defined and tags is not empty %}
											{% for item in tags %}
											<a target="_blank" class="" onclick="setTag2(this);return false;">{{ item }}</a>
											{% endfor %}
										{% endif %}
										</td>
									</tr>
	        	...
	        	if( !saveId ){
		           ...
		           if( ! $( '#categoryId' ).val() ){
		            ...
		            if( ! $( 'form span' ).hasClass( 'glyphicon-remove' ) ){ //数据正确可以提交表单了
		            	...
		            	var author = $( "#bsc_user_nickname", parent.document ).html();
		                var data = $( 'form' ).serialize();
		                var info = '&author=' + author + '&' + key + '=' + token;
		                data += info;
