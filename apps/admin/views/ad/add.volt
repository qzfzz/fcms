            ...
            <div role="tabpannel" class="tab-pane active" id="adAdd" style="padding-top:20px;">
                <form class="form-horizontal">
                    ...
                    <div class="form-group has-feedback text-right">
                        ...
                        <div class="col-xs-3">
                            <input class="form-control form_datetime" type="text" id="begin_time" name="begin_time" placeholder="请输开始时间" />
                        </div>
                        <span class="alertMsg col-xs-2"></span>
                    </div>
                    ...
                    <div class="form-group has-feedback text-right">
                        ...
                        <div class="col-xs-3">
                            <input class="form-control form_datetime" type="text" id="end_time" name="end_time" placeholder="请输结束时间" />
                        </div>
                        <span class="alertMsg col-xs-2"></span>
                    </div>
                    <div class="form-group has-feedback text-right">
                        ...
                        <div class="col-xs-3">
                            <input class="form-control" type="text" id="sort_order" name="sort_order" placeholder="请输入广告排序"/>
                        </div>
                        <span class="alertMsg col-xs-2"></span>
                    </div>
...
$( function(){
    ...
    $( '.form_datetime' ).datetimepicker({
    	...
    	if( !$( this ).parent( 'div' ).find( 'span' ).size() ){
    		$( this ).after( '<span class="glyphicon form-control-feedback"></span>' );
    	}
    	var objParent =  $( this ).parents( 'div.form-group' );
    	if( false == $( this ).val() || $( this ).val().length < 0 ){
	        $( this ).next( 'span' ).removeClass( 'glyphicon-ok' ).addClass( 'glyphicon-remove' );
	        objParent.removeClass( 'has-success' ).addClass( 'has-error' );
	        errorAlert( objParent, '请输入正确的时间' );
   		}else{
    		$( this ).next( 'span' ).removeClass( 'glyphicon-remove' ).addClass( 'glyphicon-ok' );
    ...
    /*---------------商品图片添加-------------*/
	var picEditor = UE.getEditor( 'goodsPicEdit', { isShow: false,
        toolbars: [ [ 'insertimage' ] ], bizt: 'user', serverUrl:'/common/upload/ctrl.php' } );
    ...
    $( '#sort_order' ).blur( function(){
		...
		if( pattern.test( $( this ).val() ) ){
			...
			error( objParent );
			errorAlert( objParent, '请正确输入大于0的整数' );
		}
    } );
    /*-----------添加数据-----------*/
