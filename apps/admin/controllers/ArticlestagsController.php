...
class ArticlestagsController extends AdminBaseController{
	...
	public function indexAction()
	{
		...
		$builder = $this->modelsManager->createBuilder()
					->columns( 'id,addtime,uptime,delsign,name,pinyin,display' )
					->from( 'apps\admin\models\ArticleTags' )
					->where( 'delsign='.SystemEnums::DELSIGN_NO.' ORDER BY uptime DESC,sort ASC' );
	...
	public function searchAction()
	{
		...
		if( $this->request->isAjax() && false == $keys )
		{		
			$objRet->state = 0;
			$objRet->res = array();
		}
