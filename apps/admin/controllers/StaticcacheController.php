...
class StaticcacheController extends AdminBaseController{
	...
	public function saveAction()
	{
		...
		{
			case 1://基本配置
				$index_cache_time 	= $this->request->getPost( 'ctime', 'int' );
				$list_cache_time	= $this->request->getPost( 'ltime', 'int' );
				$detail_cache_time	= $this->request->getPost( 'dtime', 'int' );
				$time_id			= $this->request->getPost( 'sign', 'int' );
				if( false != $time_id )
				{
					$where = array(
						'column'	=> 'id,delsign,index,type,list,detail',
	...
	public function columnAction()
	{
		...
		$builder = $this->modelsManager->createBuilder()
				->columns( 'id,delsign,addtime,uptime,name,cache_time,type,cfgtype,prefix ' )
				->from( 'apps\admin\models\StaticCache' )
				->where( 'delsign=' . SystemEnums::DELSIGN_NO . ' AND cfgtype=1 ORDER BY uptime DESC' );
	...
	public function listAction()
	{
		...
		$builder = $this->modelsManager->createBuilder()
				->columns( 'id,delsign,addtime,uptime,name,cache_time,type,cfgtype,prefix' )
				->from( 'apps\admin\models\StaticCache' )
				->where( 'delsign=' . SystemEnums::DELSIGN_NO . ' AND cfgtype=2 ORDER BY uptime DESC' );
	...
	public function detailAction()
	{
		...
		$builder = $this->modelsManager->createBuilder()
				->columns( 'id,delsign,addtime,uptime,name,cache_time,type,cfgtype,prefix ' )
				->from( 'apps\admin\models\StaticCache' )
				->where( 'delsign=' . SystemEnums::DELSIGN_NO . ' AND cfgtype=3 ORDER BY uptime DESC' );
