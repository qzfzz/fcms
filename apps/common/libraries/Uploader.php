...
class Uploader implements \Phalcon\Di\InjectionAwareInterface
{
    ...
    public function __construct($fileField, $config, $type = "upload", $di )
    {
        ...
        if( !$this->iEngineType )
        {
            ...
            if( 1 == intval( $this->objConfigFS->mode ) )
            {// for random fs
            ...
            else if( 0 == intval( $this->objConfigFS->mode )  )
            {
            	$objPath = new \stdClass();
            	$objPath->type	= 'local';
            	$objPath->path = $_SERVER[ 'DOCUMENT_ROOT' ] . '/public';
            	$objPath->url	= 'http://' . $_SERVER[ 'SERVER_NAME' ] . '/public';
            	
            	$this->objHitServer = $objPath;
            }
            else
