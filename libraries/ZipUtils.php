...
class ZipUtils
{
    ...
    private function file()
    {
        ...
        return $data . $ctrldir . $this->eof_ctrl_dir . pack( 'v', sizeof( $this->ctrl_dir ) ) .
                 pack( 'v', sizeof( $this->ctrl_dir ) ) . pack( 'V', strlen( $ctrldir ) ) . pack( 'V', strlen( $data ) ) .
                 "\x00\x00";
    }
    ...
    //
    // $archive = new PHPZip();
    // $archive->Zip("需压缩的文件所在目录", "ZIP压缩文件名");//压缩文件名可以带路径，即可指定压缩到哪里
    // ------------------------------------------------------ //
    public function Zip( $dir, $saveName )
    {
        ...
        {
            /*
             * //默认不会压缩空文件夹（注释掉这句话并删掉fopen和fclose的@即可不压缩空文件夹） if (! file_exists($file) || ! is_file($file)) { continue; }
             */
            $fd = @fopen( $file, "rb" );
    ...
    private function ExtractFile( $header, $to, $zip )
    {
        ...
        if( !( $header[ 'external' ] == 0x41FF0010 ) && !( $header[ 'external' ] == 16 ) )
        {
            ...
            else
            {
                ...
                $binary_data = pack( 'va1a1Va1a1', 0x8b1f, Chr( $header[ 'compression' ] ), Chr( 0x00 ), time(), 
                        Chr( 0x00 ), Chr( 3 ) );
